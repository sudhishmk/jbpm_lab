 ...
 */
public class CustomLdapSearcher {

    private static final Logger log = LoggerFactory.getLogger(CustomLdapSearcher.class);
     ...
     */
    public CustomLdapSearcher(Properties config) {
        this.config = config;
    }
    ...
    public LdapResults search(String context, String filterExpr, Object... filterArgs) {
        ...
        log.debug("##### LdapSearcher.search is called from: \n\t{}\n\t{}\n\t{}\n\t{}\n\t{}\n\t{}\n\t{}\n\t{}\n\t{}\n\t{}", Thread.currentThread().getStackTrace());
        log.debug("LdapSearcher.search context    : {}", context);
        log.debug("LdapSearcher.search filterExpr : {}", filterExpr);
        log.debug("LdapSearcher.search filterArgs : {}", filterArgs);
        ...
        try {
            ...
            log.debug(" ==> SearchResult : {}", ldapResult);
           ...
           //  start debugging
            ...
            for( SearchResult result : searchResults){ 
                log.debug("\tDN : {}",result.getNameInNamespace());
                NamingEnumeration<? extends Attribute> attrEnum = result.getAttributes().getAll();
                while (attrEnum.hasMore()) { 
                    Attribute attribute = attrEnum.next();
                    NamingEnumeration<?> valueEnum = attribute.getAll();
                    while (valueEnum.hasMore()) { 
                       log.debug("\t{} : {}", attribute.getID(), valueEnum.next());
                    }
                }
            }
            //  end debugging
